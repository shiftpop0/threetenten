...
<body>
		<div id="current">当前位置：一志愿详情&gt; ${YEAR }年${USERLIST[0].ZYMC }一志愿详情</div>
	<div class="page-container">
		<form name="SearchState" method="post"
			Action="mainpage?ActionType=mainpage_Select1Family&MAJOR=<c:out value='${requestScope.MAJOR }'/>&YEAR=<c:out value='${requestScope.YEAR }'/>">
			<div class="text-c">
				...
				<select class="select"
					name="family">
						...
						<c:forEach items="${FAMILYLIST}" var="family">
							<option value="${family.DQMC }">${family.DQMC }</option>
						</c:forEach>
				</select>
				</span> 
		...
		<h1 style="text-align:center;">${YEAR }年${USERLIST[0].ZYMC }一志愿详情</h1>
		<div class="mt-20">
			...
			<ec:table items="USERLIST" var="row"
			   ...
			   <ec:row>
        		...
        		<ec:column property="FAMILYSUM" style="text-align:center;" escapeAutoFormat="true" sortable="false" title="录取人数" />
				<%--<ec:column property="rate" style="text-align:center;" title="一志愿率"/>--%>        		
				<ec:column property="CJ" style="text-align:center;" title="平均分" />
        		<ec:column property="EDIT" style="text-align:center;" title="操作" viewsDenied="xls">
        			<a href="#" onClick="formPost(window.SearchState,'','mainpage?ActionType=mainpage_SelectAll&FAMILY=<c:out value='${row.DQDM }'/>&ZYDH=<c:out value='${row.ZYDH }'/>&YEAR=<c:out value='${row.YEAR1 }'/>&Flag=YZY',this);">详情</a> 
        		</ec:column>
				</ec:row>
			</ec:table>
		</div>

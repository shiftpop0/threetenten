...
<body>
<div id="current">当前位置：志愿详情&gt; ${YEAR }年${USERLIST[0].ZYMC }志愿详情</div>
	<div class="page-container">
		<form name="SearchState" method="post"
			Action="mainpage?ActionType=mainpage_Select2Major&YEAR=<c:out value='${YEAR }'/>&MAJOR=<c:out value='${MAJOR }'/>">
			<div class="text-c">
				<!-- 直接用了sql1查询到的结果，如果有问题重写sql2 -->
		...
		<h1 style="text-align:center;">${YEAR }年${USERLIST[0].ZYMC }志愿详情</h1>
		<div class="mt-20">
			...
			<ec:table items="USERLIST" var="row"
			   ...
			   <ec:row>
        		...
        		<ec:column property="sum" style="text-align:center;" title="人数" />
        		<%-- <ec:column property="vorate" style="text-align:center;" title="志愿率"/> --%>
				...
				<ec:column property="EDIT" style="text-align:center;" title="操作" viewsDenied="xls">
        			<a href="#" onClick="formPost(window.SearchState,'','mainpage?ActionType=mainpage_SelectAll&YEAR=<c:out value='${row.YEAR1 }'/>&ZYDH=<c:out value='${row.LQZY }'/>&FAMILY=<c:out value='${row.DQDM }'/>&Flag=ZY',this);">详情</a> 
        		</ec:column>
				</ec:row>
			</ec:table>
		</div>
